...
[개선 사항]: 최적화 시나리오 샘플링(30개) + 전체 시나리오 검증(200개) 로직 추가
"""
...
def _run_forecast_python_api(
    ...
    if not target_cols:
        raise RuntimeError(f"Target columns not found in feat.csv.")
    ...
    model = FO.build_model_pipeline("lgbm", num_cols, cat_cols, lgbm_params["tweedie_variance_power"], 0.5, lgbm_params, reg_n_jobs)
...
def run_pipeline(
    ...
    pred_all, metrics_df, residual_df_val, target_cols, dt_use = _run_forecast_python_api(
        feat_csv=feat_csv, out_pred_csv=forecast_csv, out_metrics_csv=forecast_metrics_csv,
        out_pred_by_product_csv=forecast_by_product_csv, prod_col=prod_col, dt_col=dt_col,
        best_params_path=best_params_path, seed=mc_seed
    )

    # ---------- 2.5) MC scenarios (Full 200) ----------
    if mc_scenarios and mc_scenarios > 0:
        ...
        scenarios = FO.generate_demand_scenarios(d_hat=d_hat, residuals=res_pool, n_scenarios=int(mc_scenarios), seed=int(mc_seed))
        ...
        for s in range(S):
            for i, p in enumerate(prod_list):
                for d in range(D):
                    rows.append({"scenario_id": int(s), "day_idx": int(d), prod_col: str(p), "demand": float(scenarios[s, i, d])})
    ...
    plan_df, diag = PO.optimize_plan(
        forecast_by_product=pred_df, horizons=horizons, prod_col=prod_col,
        cluster_info=cluster_info, daily_capacity=daily_capacity, lambda_smooth=lambda_smooth,
        initial_inventory=initial_inventory, int_production=int_production, scale=scale,
        min_lot_map=min_lot_map, hint_plan=hint_plan, use_cvar_obj=use_cvar_planner,
        mc_scenarios=scenarios_for_opt, cvar_alpha=float(mc_alpha), lambda_cvar=float(lambda_cvar),
        lambda_scale=int(lambda_scale), mc_wb=float(mc_wb), mc_wi=float(mc_wi),
        loss_scale=int(loss_scale), solver_seed=int(planner_seed), max_time=float(planner_max_time),
        workers=int(planner_workers), gap=float(planner_gap), log_progress=True
    )

    for c in ["demand", "produce", "end_inventory", "backlog", "shortage"]:
        if c in plan_df.columns: plan_df[c] = pd.to_numeric(plan_df[c], errors="coerce")
    if prod_col in plan_df.columns: plan_df[prod_col] = plan_df[prod_col].astype(str).str.replace(r"\.0$", "", regex=True)
    plan_df.to_csv(plan_csv, index=False, encoding="utf-8-sig")

    # ---------- 3.5) MC validation (Full 200) ----------
    if mc_scenarios and mc_scenarios > 0:
        print(f"[3.5/6] MC validating fixed plan with FULL {mc_scenarios} scenarios ...")
        scenarios_df_full = EV.load_mc_scenarios(mc_scenarios_csv) # 전체 200개 로드
        mc_validation = EV.mc_validate_plan(
            plan_df=plan_df, scenarios_df=scenarios_df_full, initial_inventory=float(initial_inventory),
            daily_capacity=float(daily_capacity), alpha=float(mc_alpha), w_b=float(mc_wb),
            w_i=float(mc_wi), fail_threshold=float(fail_threshold)
        )
        save_json(mc_validation_json, mc_validation)

    # ---------- 4-6) Metrics, Evaluator, Report (생략 없이 유지) ----------
    ...
    audit = EV.audit_and_learn(plan_df, daily_capacity, {"planning_metrics": planning_kpi}, policy_json)
    ...
    if not skip_llm:
        ...
        out = RL.build_report_with_llm(
            plan_csv=plan_csv, forecast_csv=forecast_by_product_csv, metrics_csv=forecast_metrics_csv,
            model_name=model_name, auto_regen_on_fail=True, feat_csv=feat_csv,
            mc_json=mc_validation_json, planning_metrics_json=planning_metrics_json
        )
        if isinstance(out, dict) and out.get("markdown"):
            ts = _now_str()
            out_md = os.path.join(reports_dir, f"weekly_report_{ts}.md")
            with open(out_md, "w", encoding="utf-8") as f: f.write(out["markdown"])
...
if __name__ == "__main__":
    args = parse_args()
    run_pipeline(
        data_csv=args.data, out_dir=args.out_dir, prod_col=args.prod_col, dt_col=args.dt_col,
        daily_capacity=args.daily_capacity, lambda_smooth=args.lambda_smooth,
        initial_inventory=args.initial_inventory, int_production=args.int_production,
        model_name=args.model, policy_path=args.policy_path, skip_llm=args.skip_llm,
        best_params_path=args.best_params_path, mc_scenarios=args.mc_scenarios,
        mc_alpha=args.mc_alpha, mc_seed=args.mc_seed, mc_wb=args.mc_wb, mc_wi=args.mc_wi,
        fail_threshold=args.fail_threshold, scale=args.scale, use_cvar_planner=args.use_cvar_planner,
        hint_plan_csv=args.hint_plan_csv, lambda_cvar=args.lambda_cvar,
        lambda_scale=args.lambda_scale, loss_scale=args.loss_scale,
        planner_max_time=args.planner_max_time, planner_gap=args.planner_gap,
        planner_workers=args.planner_workers, planner_seed=args.planner_seed,
